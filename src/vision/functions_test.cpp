...
Mat findBiggestBlob(Mat src)
{
    ...
    Mat dst(src.rows,src.cols,CV_8UC1,Scalar::all(0)); // dst is a black canvas at this point?
    src.copyTo(temp);
...
void findGrass(Mat src, Mat HSV) //this should be separated into a few more readable functions
{
    ...
    // find all points in the image that fall into the range of grass's color
    ...
    // find the largest (grass) blob
    imgThresholded = findBiggestBlob(imgThresholded);
    
    // in the original picture, set any non-grass colors to black
    ...
    /*
    	go through each column, showing anything between two grass colors by column
    */
    for(int i = 0; i < temp.cols - 1; i++)
    {
        // now searching through one column for the area between the topmost and bottom-most grass points
    ...
    // next we want to make black all the points outside of the leftmost and rightmost grass points ??
...
bool beaconLocation(vector<KeyPoint> imgKeyPoints, beacon_loc *b_loc) 
{
    ...
    if(!b_loc->beacon_not_found) {//if you find beacon calculate rvec and tvec
    	...
    	rvec=(Mat_<double>(3,1)<<   0,
	      ...
	      0);//calculated based on information from nav team

    	tvec=(Mat_<double>(3,1)<<   -1*b_loc->x,
                                    6, 
	      -1*b_loc->y);// calculated based on information from nav team
	...
	//Mat rvec, tvec;
    bool succ = solvePnP(Mat(kwnPoints), Mat(imgPoints), cameraMatrix, distCoeffs, rvec, tvec, guess);//checks to see if object was found
