...
// ROS callback function for a beacon request being published. This does what BeaconRequest_hand does
...
{
	ros::init(argc, argv, "robot_sim"); // Initialize ROS
	ros::NodeHandle n;
	//response_pub =n.advertise<robot::BeaconResponse>("BeaconResponse",1000); // Publish beacon response
	response_pub = n.advertise<robot::BeaconRequest>("BeaconRequest", 1000);
	ros::Subscriber request_sub = n.subscribe("BeaconResponse", 1000, beaconCallback);
